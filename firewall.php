...
if(empty($sleeptime) || empty($waittime))
{
    $file = new file;
    $time = date("Y-m-d H:i:s");
...
while (true) {
    ...
    if (!$ifunderattack) {
        if ($load_one < $load_safe*0.6) {
        ...
        } else if ($load_one >= $load_safe*0.9 && $load_one < $load_safe) {
            echo "[caveat]$time | 当前负载: " . $load_one . "即将到达安全负载: " . $load_safe . " -> 尝试开盾\n";
            ...
            echo "[underattack]$time | 当前负载: " . $load_one . "超过安全负载: " . $load_safe . " -> 尝试开盾\n";
