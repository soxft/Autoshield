...
class bt_main {
    ...
    public function addRecordtoLocal($domain,$id,$type,$name,$content,$proxied,$ttl)
    {
      $f = new file();
...
//cloudflare设置防御模式
class setsecurity extends post {
...
class dnscontrol extends post {
    ...
    function getdnslist() {
        ...
        if (file_exists("config/domaindns/" . $this -> domain . ".json")) {
            ...
            $return = json_decode(self::postgo($url,"GET","",$this->cfkey,$this->cfemail),true)['result'];
            //开始遍历
            @$num = count($return);
            $data = array();
            for ($i = 0;$i <= $num-1;$i++) {
                ...
                //字符超出字数，则处理下
                if(strlen($data[$i]['content']) > 25)
                {
                    $data[$i]['content']  = substr($data[$i]['content'],0,25) . "...";
                }
                if(strlen($data[$i]['name']) > 20)
                {
                    $data[$i]['name']  = substr($data[$i]['name'],0,20) . "...";
                }
            }
    ...
    function addDns ($type,$name,$content,$ttl,$proxied)
    {
      $data = array(
        ...
        $url = "https://api.cloudflare.com/client/v4/zones/$this->zone/dns_records";
        $return = self::postgo($url,"POST",$data,$this->cfkey,$this->cfemail);
    ...
    function editDns($id,$type,$name,$content,$ttl,$proxied)
    {
      $data = array(
        ...
        $return = self::postgo($url,"PUT",$data,$this->cfkey,$this->cfemail);
        $return = json_decode($return,true);
        if($return['success']){
          return array(
            "code" => 200,
    ...
    {
        $url = "https://api.cloudflare.com/client/v4/zones/$this->zone/dns_records/$id";
        $return = self::postgo($url,"DELETE","",$this->cfkey,$this->cfemail);
        $return = json_decode($return,true);
        if($return['success']){
                //接下来删除本地解析记录
...
//cloudflare获取站点防御信息类
class getsercuity extends post {
    ...
    function getsecurity() {
        ...
        for ($i = 0;$i <= $num - 1;$i++) {
            $id = $this -> zone[$i]['zone'];
            $url = "https://api.cloudflare.com/client/v4/zones/$id/settings/security_level";
            $arr[$i]['security'] = json_decode(self::postgo($url,"GET","",$this->cfkey,$this->cfemail),true)['result']['value'];
...
class checkcloudflare extends post {
    ...
    function getzone() {
        ...
        $zones = self::postgo($url,"GET","",$this->cfkey,$this->cfemail);
...
class post
{
    public function postgo($url = "",$method = "GET",$data = array(),$apikey,$email) {
        $headers = array(
